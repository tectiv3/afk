...
Examples:
  ...
  afk telegram test      # Test Telegram connection`
    ...
    formatUninstallInstructions(scope) {
        if (scope === 'user') {
            return 'To uninstall, remove hook entries from ~/.claude/settings.json `hooks` section and delete ~/.config/afk.'
        } else {
            return "To uninstall, remove hook entries from your project's .claude/settings(.local).json."
        }
    }
}
