...
class SessionsService {
    ...
    constructor(configManager, logger, utils) {
        ...
        this.logger = logger
        this.utils = utils
        const cfgDir = path.join(process.env.HOME || process.env.USERPROFILE, '.config', 'afk')
        this.activeSessionsPath = path.join(cfgDir, 'active-sessions.json')
        this.sessionMapPath = path.join(cfgDir, 'session-map.json')
        this.replyLockPath = path.join(cfgDir, 'reply-lock.json')
        this.historyPath = path.join(cfgDir, 'history.jsonl')
    }
