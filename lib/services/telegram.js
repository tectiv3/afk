...
class TelegramService {
     ...
     */
    async sendMessage(text, reply_markup = null, parseMode = 'Markdown') {
    ...
    async sendDocument(filePath, caption = '', reply_markup = null) {
        ...
        parts.push(`--${boundary}\r\n`)
        parts.push(
            `Content-Disposition: form-data; name="document"; filename="${path.basename(
                filePath
            )}"\r\n`
        )
                ...
                res.on('end', () => {
                    try {
                        ...
                        if (response.ok) {
                            ...
                            const error = new Error(
                                response.description || 'Failed to send document'
                            )
                            error.error_code = response.error_code
                            reject(error)
                        }
