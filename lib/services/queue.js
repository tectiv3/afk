...
class QueueService {
    ...
    async distributedTelegramPoll(
        ...
        this.logger.eprint(
            `[${hookId}] Starting distributed polling for session ${sessionId}...`
        )
        ...
        while (Date.now() - startTime < timeoutMs) {
            try {
                ...
                if (updates.length > 0) {
                    ...
                    if (newUpdates.length > 0) {
                        ...
                        this.logger.eprint(
                            `[${hookId}] Dumped ${
                                newUpdates.length
                            } new messages to global queue (${
                                updates.length - newUpdates.length
                            } duplicates skipped)`
                        )
                    } else {
                        this.logger.eprint(
                            `[${hookId}] All ${updates.length} messages were duplicates, skipping`
                        )
                    }
                }
                ...
                if (claimedMessage) {
                    this.logger.eprint(
                        `[${hookId}] Claimed message ${claimedMessage.update_id}`
                    )
