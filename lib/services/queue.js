...
class QueueService {
    ...
    constructor(configManager, logger, utils) {
        ...
        this.queueDir = path.join(cfgDir, 'messages')
        ...
        while (Date.now() - startTime < timeoutMs) {
            try {
                ...
                if (updates.length > 0) {
                    ...
                    if (newUpdates.length > 0) {
                        ...
                        this.logger.debugLog(
                            'POLL',
                            `${hookId}: queued ${newUpdates.length} new`
                        )
                    } else {
                        this.logger.debugLog(
                            'POLL',
                            `${hookId}: ${updates.length} duplicates skipped`
                        )
                    }
                }
                ...
                if (claimedMessage) {
                    this.logger.debugLog(
                        'POLL',
                        `${hookId}: claimed ${claimedMessage.update_id}`
                    )
