...
class ClaudeHooksService {
    ...
    /**
     * Strip markdown syntax for plain-text display in Telegram HTML messages
     */
    stripMarkdown(text) {
        if (!text) return ''
        return text
        ...
        try {
            ...
            for (const line of lines) {
                try {
                    ...
                    // Strip <system-reminder> blocks (hook output injected into transcript)
                    text = text.replace(/<system-reminder>[\s\S]*?<\/system-reminder>/g, '')
                    // Strip stop hook stderr noise (lines like "Stop hook feedback:", emoji prefixed hook logs)
                    text = text.replace(/^.*Stop hook (?:feedback|error):.*$/gm, '')
                    text = text.replace(/^.*\[afk\] Stop hook.*$/gm, '')
                    text = text.replace(/^\[.*?afk hook stop\].*$/gm, '')
                    text = text.replace(/^\[stop-[a-f0-9-]+\].*$/gm, '')
                    // Extract clean user text from hook injection pattern
                    const replyMatch = text.match(/User replied via Telegram: "(.+?)"\.\s*Continue the conversation/s)
                    if (replyMatch) {
                        text = replyMatch[1]
                    }
                    text = text.replace(/\n{3,}/g, '\n\n').trim()
    ...
    _buildContextSection(context) {
        ...
        if (context.recentMessages && context.recentMessages.length > 0 && !context.error) {
            ...
            for (const msg of messagesToShow) {
                ...
                const safeText = this.escapeHtml(this.stripMarkdown(msg.text)).substring(0, 500)
                contextSection += `\n${roleIcon} ${safeText}`
            }
        }
