...
class ClaudeHooksService {
    ...
    async handleQuestion(data) {
        ...
        for (let qi = 0; qi < questions.length; qi++) {
            ...
            const optionButtons = options.map((opt, i) => {
                const btnLabel = opt.label.length > 30 ? opt.label.substring(0, 28) + '..' : opt.label
                return { text: `${i + 1}. ${btnLabel}`, callback_data: `qopt:${approvalId}:${i}` }
            })
            ...
            if (questions.length > 1) {
                try {
                    ...
                    await this.telegramService.tgApiWithToken(token, 'editMessageText', {
                        chat_id: chatId,
                        message_id: messageId,
                        text: `[${label}] ✅ <b>${this.escapeHtml(q.header || 'Question')}</b> — ${this.escapeHtml(String(answer))}`,
                        parse_mode: 'HTML',
                    })
                } catch (e) {
    ...
    async sendStopNotificationReadOnly(sessionId, label, cwd, context) {
        ...
        const text = `[${this.escapeHtml(
            label
        )}] 📖 <b>Session completed</b> <i>(read-only)</i>${contextSection}\n\n<code>${this.shortSession(
            sessionId
        )}</code>`
