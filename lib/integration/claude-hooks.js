...
class ClaudeHooksService {
    ...
    async processToolApproval(update, approvalId, sessionId) {
        ...
        if (decision === 'approve') {
            ...
            const approvalText = `✅ *Approved* — ${this.formatToolDisplay(
                toolName,
                toolInput
            )}`
            ...
            const reasonText =
                addedCount > 0
                    ? `Approved via Telegram and added ${addedCount} new ${
                          addedCount === 1 ? 'pattern' : 'patterns'
                      } to permissions` +
            ...
            const resultText =
                `✅ *Allowed All* — ${this.formatToolDisplay(
                    toolName,
                    toolInput
            ...
            const delegateText = `🔧 *Delegating to Claude UI* — ${this.formatToolDisplay(
                toolName,
                toolInput
            )}`
    ...
    async sendApprovalRequest(
        ...
        // Escape summary for Markdown (commands may contain _ * ` etc.)
        const safeSummary = summary.replace(/[_*`[\]]/g, '\\$&')

        const text = `🤖 *Approval required* — ${label}\n${safeSummary}${contextSection}\n\n_Pattern${
            Array.isArray(patterns) ? 's' : ''
        }:_ \`${patternText}\`\n_Session:_ \`${this.shortSession(
            sessionId
        )}\`\n_Dir:_ \`${cwd}\``
