...
class ClaudeHooksService {
  ...
  async processToolApproval(update, approvalId, sessionId) {
    ...
    if (decision === 'approve') {
      ...
      // No output = fall through to normal Claude UI permission prompt
      return {};
    }
  ...
  async waitForApproval(approvalId, sessionId, toolName, toolInput) {
    ...
    if (timeoutAction === 'allow') {
      ...
      return {
        hookSpecificOutput: {
          hookEventName: 'PermissionRequest',
          decision: { behavior: 'deny', message: `Auto-denied after ${timeout}s timeout` }
        }
      };
    }

    // Fall through to normal Claude UI prompt
    return {};
  }
