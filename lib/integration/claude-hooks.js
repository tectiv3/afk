...
class ClaudeHooksService {
    ...
    async processToolApproval(update, approvalId, sessionId) {
        ...
        const { patterns = [], toolName = '', toolInput = {}, cwd = '', permissionSuggestions = [] } = metadata
    ...
    async handlePermissionRequest(data = null) {
        ...
        fs.writeFileSync(
            metaFile,
            JSON.stringify({ patterns, toolName, toolInput, cwd, sessionId, permissionSuggestions })
        )
    ...
    async sendStopNotification(sessionId, label, cwd, context) {
        ...
        const text = `[${this.escapeHtml(label)}] ✅ <b>Agent finished</b>${contextSection}\n\n<code>${this.shortSession(sessionId)}</code>\n\nTap <b>Reply</b> to send a follow-up, or <b>Finish</b> to close.`
    ...
    async sendStopNotificationReadOnly(sessionId, label, cwd, context) {
        ...
        const text = `[${this.escapeHtml(label)}] 📖 <b>Session completed</b> <i>(read-only)</i>${contextSection}\n\n<code>${this.shortSession(sessionId)}</code>`
    ...
    async sendSessionStartNotificationReadOnly(sessionId, label, cwd, source) {
        ...
        const text = `[${this.escapeHtml(label)}] ${sourceText} <i>(read-only)</i>\n<code>${this.shortSession(sessionId)}</code> · <code>${this.escapeHtml(cwd)}</code>`
